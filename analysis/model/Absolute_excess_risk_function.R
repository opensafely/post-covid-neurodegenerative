## =============================================================================
## Project:     Post-covid-events (Vaccinated delta wave population study)
##
## Purpose:  
...
# # CODE FOR SINGLE AER
# event_of_interest="REM, vascular dementia"
...
excess_risk <- function(outcome_of_interest, cohort_of_interest, model_of_interest, analysis_of_interest, input) {  # subgroup_of_interest, 
  ...
  input <- input %>% mutate(across(c("hr","unexposed_person_days","unexposed_events","total_exposed", "sample_size"), as.numeric)) 
  ...
  lifetable$incidence_unexp <- input$unexposed_events[1] / input$unexposed_person_days[1]
  ...
  #lifetable$N_exposed <- input$total_exposed[1] 
  #lifetable$AER <- lifetable$excess_risk * lifetable$N_exposed  #<- REMOVED (Cc)
  ...
  # 
  # results <- data.frame(outcome = outcome_of_interest,
  #                       cohort = cohort_of_interest,
  #                       model = model_of_interest,
  #                       AER_followup_end = AER_followup_end) #<-
  # 
  # write.csv(results, paste0(aer_raw_results_dir, "/AER_raw_results_", cohort_of_interest, "_", model_of_interest, "_", outcome_of_interest,".csv"), row.names = F)
  # return(results)
}