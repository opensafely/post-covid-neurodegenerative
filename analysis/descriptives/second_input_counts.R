...
prevax<-readr::read_rds("output/input_prevax_stage1.rds")
vax<-readr::read_rds("output/input_vax_stage1.rds")
unvax<-readr::read_rds("output/input_unvax_stage1.rds")
...
write.table(count_df,quote=F,row.names=T,col.names=T,"output/study_counts_stage1clean.txt")

#summary data 
describe_data <- function(data) {
  file_name <- paste0("output/describe_stage1clean_", deparse(substitute(data)), ".txt")
  sink(file_name)
